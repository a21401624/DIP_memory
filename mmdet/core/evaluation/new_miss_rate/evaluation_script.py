...
class KAIST(COCO):
    ...
    def loadRes(self, resFile):
        # Modified by Yuxuan Hu on 2022/12/9.
        """
        ...
        if type(resFile) == str and resFile.endswith('.txt'):
            ...
            for i in range(len(resFile)):
                if resFile[i][0].shape[0] != 0:
                    for j in range(resFile[i][0].shape[0]):
                        bbox = resFile[i][0][j]
                        x_tl = bbox[0]
                        y_tl = bbox[1]
                        w = bbox[2] - bbox[0]
                        h = bbox[3] - bbox[1]
                        json_format = {}
                        json_format["image_id"] = i                          
                        json_format["category_id"] = 1
                        json_format["bbox"] = [x_tl, y_tl, w, h]
                        json_format["score"] = bbox[4]
                        _resFile.append(json_format) 
            res = super().loadRes(_resFile)
        else:
...
def eval_LAMR(metrics: List[str], 
              ...
              logger):
    """
        metrics: list including 'MR_all', 'MR_day' and 'MR_night'.
        results: list of [np.array], each array has shape (N, 5), 
                 (x_tl, y_tl, x_rb, y_rb, confidence).
        ground_truth: list of dict, each dict has keys 
                      'bboxes', 'labels', 'gt_ignore'.
        img_list: txt file path of full dataset.
        img_ids: list of image names.
    """
    dataset = {}
    ...
    id = 0
    image_id = 0
    for img_name, ann in zip(img_ids, ground_truth):
        temp = {'id': image_id, 'im_name': img_name, 
                'height': 512, 'width': 640}
        images.append(temp)
        for idx in range(len(ann['labels'])):
            temp = {
                'id': id,
                'image_id':image_id,
                'category_id': 1,
                'bbox': [ann['bboxes'][idx][0], ann['bboxes'][idx][1],
                    ann['bboxes'][idx][2]-ann['bboxes'][idx][0], 
                    ann['bboxes'][idx][3]-ann['bboxes'][idx][1]
                ],
                'height': ann['bboxes'][idx][3]-ann['bboxes'][idx][1],
                "occlusion": ann['occlusions'][idx],
                'ignore': int(ann['gt_ignore'][idx])
            }
    ...
    categories = []
    categories.append({'id': 0, 'name': '__ignore__'})
    categories.append({'id': 1, 'name': 'person'})
    categories.append({'id': 2, 'name': 'cyclist'})
    categories.append({'id': 3, 'name': 'people'})
    categories.append({'id': 4, 'name': 'person?'})

    dataset.update(images=images, 
                   annotations=annotations, 
                   categories=categories)

    kaistGt = KAIST(dataset)
    kaistDt = kaistGt.loadRes(results)

    imgIds = sorted(kaistGt.getImgIds()) # sorted return List.
    ...
    results = {}
    return_result = {}
    for i in ['all', 'day', 'night']:
        if 'MR_' + i not in metrics:
            continue
        eval_result = copy.deepcopy(kaistEval)
        if i == 'all':
            eval_result.params.imgIds = imgIds
        # As imgIds are arranged in order from day to night, we take a shortcut.
        elif i =='day':
            with open(img_list.replace(".txt", "_day.txt"), 'r') as f:
                day_list = f.readlines()
            day_len = len(day_list)
            eval_result.params.imgIds = imgIds[:day_len]
        elif i =='night':
            with open(img_list.replace(".txt", "_day.txt"), 'r') as f:
                day_list = f.readlines()
            day_len = len(day_list)
        ...
        eval_result.evaluate(0)
        eval_result.accumulate()
        LAMR = eval_result.summarize(0) * 100
        MR = eval_result.eval['yy'][0][-1]
        FPPI = eval_result.eval['xx'][0][-1]
        results['LAMR_' + i] = LAMR
        results['MR_' + i] = MR
        results['FPPI_' + i] = FPPI
        return_result['MR_' + i] = LAMR

    print_mr_summary(results, logger=logger)

    return return_result

def print_mr_summary(results: dict, logger=None):
    header = ['', 'LAMR', 'MR', 'FPPI']
    table_data = [header]

    rows = []
    if 'LAMR_all' in results.keys():
        rows.append('all')
    if 'LAMR_day' in results.keys():
        rows.append('day')
    if 'LAMR_night' in results.keys():
        rows.append('night')
    for j in rows:
        LAMR = results['LAMR_'+j]
        MR = results['MR_'+j]
        FPPI = results['FPPI_'+j]
        row_data = [
            j, f'{LAMR:.2f}', f'{MR:.3f}', f'{FPPI:.3f}'
        ]
